...
<head>
  ...
  <title>Edit Medical Record - <?= esc($record['record_number'] ?? '#' . $record['id']) ?></title>
...
<header class="dash-topbar" role="banner">
  <div class="topbar-inner">
    <a href="<?= site_url('doctor/records/' . $record['id']) ?>" class="menu-btn" aria-label="Back"><i class="fas fa-arrow-left"></i> Back</a>
    <div class="brand">
      <img src="<?= base_url('assets/img/logo.png') ?>" alt="HMS" />
      <div class="brand-text">
        <h1 style="font-size:1.25rem;margin:0">Edit Medical Record</h1>
  ...
  <main class="content">
    <section class="panel">
      <div class="panel-head">
        <h2 style="margin:0;font-size:1.1rem">Edit Medical Record</h2>
      </div>
      <div class="panel-body">
        ...
        <form method="post" action="<?= site_url('doctor/records/' . $record['id']) ?>" style="max-width:900px;">
          ...
          <div style="display:flex;gap:10px;justify-content:flex-end;padding-top:16px;border-top:1px solid #e5e7eb;">
            <a href="<?= site_url('doctor/records/' . $record['id']) ?>" class="btn" style="padding:10px 20px;background:white;border:1px solid #d1d5db;border-radius:6px;text-decoration:none;color:#374151;font-weight:500;">
              Cancel
            </a>
