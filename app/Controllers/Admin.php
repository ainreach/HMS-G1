...
class Admin extends BaseController
{
    ...
    public function staffSchedules()
    {
        ...
        $schedules = $scheduleModel
            ...
            ->groupStart()
                ->where('staff_schedules.start_date <=', $endDate)
                ->where('staff_schedules.end_date >=', $startDate)
            ->groupEnd()
        ...
        // Get rest days for each staff member
        $restDays = [];
        $staffSchedules = $scheduleModel
            ->select('user_id, rest_day')
            ->where('rest_day IS NOT NULL')
            ->where('is_active', 1)
            ->findAll();

        foreach ($staffSchedules as $schedule) {
            $restDays[$schedule['user_id']] = $schedule['rest_day'];
        }
    ...
    public function storeStaffSchedule()
    {
        ...
        $restDay = $this->request->getPost('rest_day');
        $startDate = $this->request->getPost('start_date');
        $endDate = $this->request->getPost('end_date');
        $isRecurring = (bool)$this->request->getPost('is_recurring');

        // Basic validation
        if (!$userId || $startTime === '' || $endTime === '' || !$startDate || !$restDay) {
            return redirect()->back()->with('error', 'Staff, rest day, date range, and time slots are required.')->withInput();
        }
        ...
        if (strtotime($endDate) < strtotime($startDate)) {
            return redirect()->back()->with('error', 'End date must be after start date.')->withInput();
        }

        // Ensure end date is not more than 1 year from start date
        $maxEndDate = date('Y-m-d', strtotime($startDate . ' +1 year'));
        if (strtotime($endDate) > strtotime($maxEndDate)) {
            return redirect()->back()->with('error', 'Schedule cannot exceed 1 year.')->withInput();
        }
        ...
        // Remove rest day from the days to schedule
        $daysToSchedule = array_diff($daysOfWeek, [$restDay]);
        
        // Check for existing schedule conflicts for any day in the date range
        $existing = $scheduleModel
            ->where('user_id', $userId)
            ->whereIn('day_of_week', $daysToSchedule)
            ->where('is_active', 1)
            ->groupStart()
        ...
        if ($existing) {
            return redirect()->back()->with('error', 'A schedule already exists for this staff member during the selected date range.')->withInput();
        }
        ...
        try {
            // First, deactivate any existing schedules for this user
            $scheduleModel
                ->where('user_id', $userId)
                ->set(['is_active' => 0])
                ->update();
            
            // Set rest day for the user
            $scheduleModel
                ->where('user_id', $userId)
                ->set('rest_day', $restDay)
                ->update();
            
            // Create schedules for each day except rest day
            foreach ($daysToSchedule as $day) {
                ...
                if (!$scheduleModel->save($data)) {
                    throw new \RuntimeException('Failed to save schedule for ' . $day);
                }
            }
            ...
            $db->transRollback();
            return redirect()->back()->with('error', 'Failed to save schedule: ' . $e->getMessage())->withInput();
        }

        return redirect()->back()->with('error', 'Failed to add schedule.')->withInput();
    }
